  ...
  const getEntityEntities = createSelector(
    ...
    (
      ...
      return Object.fromEntries(
        Object.entries(entitiesDictionary).map(([key, value]) => [key, value])
...
>(featureKey: string, entityAdapter: EntityAdapter<EntityType>) {
  ...
  const getSelectedEntity = createSelector(
    ...
    (entitiesDictionary, id) => {
      ...
      if (
        Number.isNaN(+id) &&
        entitiesArray[0] !== undefined &&
        'key' in (entitiesArray[0] as any)
      ) {
        const entitiesArray = Object.values(entitiesDictionary);
        const matchingEntity = entitiesArray.find(
          (entity: any) => (entity as EntityType & { key: string }).key === id
        );
