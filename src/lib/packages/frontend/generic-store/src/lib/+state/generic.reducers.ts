...
export function createGenericReducer<EntityType>(
  entityName: string,
  actions: ReturnType<typeof createGenericActions>,
  entityAdapter: EntityAdapter<EntityType>
): ActionReducer<EntityState<EntityType>, Action> {
  {
    return createReducer<EntityState<EntityType>>(
      ...
      on(
        actions.loadEntitiesFilteredSuccess,
        (state, { payload: paginated }) => {
      ...
      on(actions.addEntitySuccess, (state, { payload }) => {
        const entity: EntityType = payload.entity as EntityType;
...
export function createGenericReducerState<
  EntityType,
...
>(
  ...
  actions: ReturnType<typeof createGenericActions>,
  entityAdapter: EntityAdapter<EntityType>
): ActionReducer<EntityData, Action> {
  {
