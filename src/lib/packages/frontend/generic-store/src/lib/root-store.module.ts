...
import { GenericData, getStoreFeatures, NotificationData } from '@nxtlvls/generic-store';
...
export class RootStoreModule {
  static forFeature(model: Type<any>): ModuleWithProviders<RootStoreModule> {
    ...
    const apiUrl = `${environment.baseUrl}/${config.url}`; // wird später aktualisiert

    const store = getStoreFeatures<typeof model, {
