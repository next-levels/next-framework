...
var BatchWizardComponent = function () {
    ...
    var BatchWizardComponent = _classThis = /** @class */ (function () {
        function BatchWizardComponent_1(swalService, _matDialog, minimizeService, progress) {
        ...
        BatchWizardComponent_1.prototype.ngOnInit = function () {
            var _this = this;
            this.socket = (0, socket_io_client_1.io)('/batch');
            this.socket.on('batch', function (progressCount) {
                if (progressCount === 100) {
                    _this.progressRef.complete();
                    _this._matDialog.closeAll();
                }
                _this.progressRef.set(progressCount);
            });
        };
        BatchWizardComponent_1.prototype.ngAfterViewInit = function () {
            var _a, _b;
            if (!this.formController) {
                this.formController = new src_1.FormController((_a = this.values) !== null && _a !== void 0 ? _a : this.model, this.state, this.model);
            }
        ...
        BatchWizardComponent_1.prototype.getSteps = function (fileFields) {
            var currentStep = {
                index: 1,
            ...
            for (var i = 0; i < fileFields.length; i++) {
                var field = Reflect.getMetadata(src_2.VISIBILITY_PREFIX, this.model, fileFields[i]);
                currentStep.fields.push(fileFields[i]);
            }
            this.steps.push(currentStep);
        };
        BatchWizardComponent_1.prototype.nextStep = function () {
            if (this.currentStep !== this.steps.length)
                this.currentStep++;
            this.formStepValid = false;
        };
        BatchWizardComponent_1.prototype.prevStep = function () {
            if (this.currentStep !== 0)
                this.currentStep--;
        };
        BatchWizardComponent_1.prototype.fireAction = function () {
            var form = this.formController.getForm().value;
            if (this.scope) {
                form[this.scope.key] = this.scope.value;
            }
            if ('batchEdit' in this.modelFacade) {
                var cleanForm = src_2.UTIL.removeNullProperties(form);
                var ids = this.values.map(function (obj) { return obj.id; });
        ...
        BatchWizardComponent_1.prototype.onDismiss = function () {
            var _this = this;
        ...
        return BatchWizardComponent_1;
    }());
    __setFunctionName(_classThis, "BatchWizardComponent");
    (function () {
        ...
        BatchWizardComponent = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return BatchWizardComponent = _classThis;
}();
